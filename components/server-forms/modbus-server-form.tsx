...
import { 
  ...
  mapFrontendRegisterTypeToFunctionCode,
  mapFrontendDataTypeToModbusType,
  determineRegisterAccess,
  type ModbusMapping,
  type DataServiceResponse
} from "@/lib/api/data-service"

// Import CSV integration
import { ModbusCSVIntegration, type ModbusServerTag, getSampleCSVContent } from './modbus-csv-integration'

// Modbus TCP Server configuration schema (aligned with Data-Service)
const serverConfigSchema = z.object({
  ...
  port: z
    ...
    .default(5020), // Data-Service uses port 5020 by default
})
...
export default function ModbusTcpServerForm() {
  ...
  const [nextAddress, setNextAddress] = useState(40001) // Start from Holding Register range
  ...
  const checkDataServiceConnection = useCallback(async () => {
    ...
    try {
      ...
      if (healthResponse.ok) {
        ...
        const actualEnabled = healthResponse.data.services?.modbus?.running || false
        const actualPort = healthResponse.data.services?.modbus?.port || 5020
  ...
  const syncExistingMappings = useCallback(async () => {
    try {
      const mappingsResponse = await dataServiceAPI.getModbusMappings()
      if (mappingsResponse.ok) {
        const mappings = mappingsResponse.data || {}
        setExistingMappings(mappings)
        
        // Convert existing mappings to server tags for display
        const tags: ModbusServerTag[] = Object.entries(mappings).map(([dataId, mapping]: [string, any]) => {
          // Map function code to register type
          const getRegisterType = (fc: number): ModbusServerTag["registerType"] => {
            switch (fc) {
              case 1: return 'Coil'
              case 2: return 'Discrete Input'
              case 3: return 'Holding Register'
              case 4: return 'Input Register'
              default: return 'Holding Register'
            }
          }
          ...
          return {
            ...
            defaultValue: 0,
            modbusAddress: mapping.register_address,
            registerType: getRegisterType(mapping.function_code),
            functionCode: mapping.function_code || 3,
  ...
  const onSubmit = async (values: ServerFormValues) => {
    ...
    try {
      ...
      if (isConnected) {
        if (values.enabled) {
          // Enable the Modbus service
          const enableResponse = await dataServiceAPI.enableModbusService()
          if (isDataServiceError(enableResponse)) {
            throw new Error(enableResponse.error)
          }
          
          toast.success("Modbus TCP Server enabled via Data-Service")
        } else {
          // Disable the Modbus service
          const disableResponse = await dataServiceAPI.disableModbusService()
          if (isDataServiceError(disableResponse)) {
            throw new Error(disableResponse.error)
          }
          
          toast.success("Modbus TCP Server disabled via Data-Service")
        }
        ...
        setConfig(next)
        toast.success("Modbus TCP Server configuration saved locally")
      }
    } catch (error: any) {
  ...
  const handleAddTags = async (selectedTags: any[]) => {
    ...
    try {
      if (!isConnected) {
        ...
        const newTags: ModbusServerTag[] = selectedTags.map((selectedTag, index) => ({
          id: selectedTag.id,
          tagName: selectedTag.name,
          tagType: selectedTag.type || 'IO',
          dataType: selectedTag.dataType || 'Analog',
          defaultValue: selectedTag.defaultValue || 0,
          modbusAddress: nextAddress + index,
          registerType: selectedTag.dataType === 'Digital' ? 'Coil' : 'Holding Register',
  ...
  const handleSingleTagRegistration = async (selectedTag: any) => {
    ...
    // Step 2: Create Modbus mapping
    const registerType = selectedTag.dataType === 'Digital' ? 'Coil' : 'Holding Register'
    const functionCode = mapFrontendRegisterTypeToFunctionCode(registerType)
    const modbusDataType = mapFrontendDataTypeToModbusType(selectedTag.dataType || 'Analog')
    const access = determineRegisterAccess(registerType)
    
    const mappingResponse = await dataServiceAPI.createModbusMapping({
      id: dataId,
      key: generateDataServiceKey(selectedTag),
      register_address: nextAddress,
    ...
    toast.success(`Tag "${selectedTag.name}" added to Data-Service Modbus server`)
  ...
  const handleBulkTagRegistration = async (selectedTags: any[]) => {
    ...
    // Step 2: Process results and create Modbus mappings for successful registrations
    const bulkResult = bulkRegisterResponse.data
    const newTags: ModbusServerTag[] = []
    ...
    for (let i = 0; i < selectedTags.length; i++) {
      ...
      if (result && result.ok) {
        // Create Modbus mapping for this successfully registered tag
        const registerType = selectedTag.dataType === 'Digital' ? 'Coil' : 'Holding Register'
        const functionCode = mapFrontendRegisterTypeToFunctionCode(registerType)
        const modbusDataType = mapFrontendDataTypeToModbusType(selectedTag.dataType || 'Analog')
        const access = determineRegisterAccess(registerType)
        ...
        try {
          ...
          console.warn(`Error creating Modbus mapping for ${selectedTag.name}:`, mappingError)
        }
      } else {
    ...
    if (bulkResult.successful > 0 && bulkResult.failed === 0) {
      toast.success(`Successfully added ${bulkResult.successful} tags to Data-Service Modbus server`)
  ...
  const getRegisterTypeOptions = () => [
    { value: "Coil", label: "Coil (FC=1, Boolean Output)", functionCode: 1 },
    { value: "Discrete Input", label: "Discrete Input (FC=2, Boolean Input, Read-Only)", functionCode: 2 },
    { value: "Holding Register", label: "Holding Register (FC=3, Read/Write)", functionCode: 3 },
    { value: "Input Register", label: "Input Register (FC=4, Read-Only)", functionCode: 4 },
  ]
  ...
  const downloadSampleCSV = () => {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', 'modbus-tags-sample.csv')
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {isConnected 
                  ? `Connected to Data-Service on port ${serverHealth?.services?.modbus?.port || 5020}. Server is ${serverHealth?.services?.modbus?.running ? 'running' : 'stopped'}.`
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    {isConnected 
                      ? "Toggle Data-Service Modbus server" 
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        {isConnected ? "Port is managed by Data-Service (MODBUS_PORT env var)" : "Default is 5020"}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Map single or multiple tags to Modbus register addresses for client access
        ...
        <CardContent>
          {serverTags.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">Click "Add Tags" to map single or multiple tags to Modbus addresses</p>
              {isConnected && (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-[200px]">Register Type</TableHead>
                    ...
                    <TableRow key={tag.id}>
                          ...
                          onValueChange={(value) => {
                            ...
                            if (option) {
                              handleTagUpdate(tag.id, {
                                registerType: value as ModbusServerTag["registerType"],
                                functionCode: option.functionCode,
                                access: determineRegisterAccess(value) as "r" | "rw"
                              })
                            }
                          }}
                      ...
                      <TableCell>
                        <Input
                          ...
                          onChange={(e) => handleTagUpdate(tag.id, { modbusAddress: Number(e.target.value) })}
                          className="w-[150px]"
                          min={1}
                          disabled={isConnected && tag.dataId} // Disable if managed by Data-Service
                        />
