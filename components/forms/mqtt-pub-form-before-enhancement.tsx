...
export function MQTTPubForm() {
  ...
  const testPublish = async (topicId: string) => {
    const mapping = mappings.find(m => m.id === topicId)
    if (!mapping) return

    try {
      toast.info(`Testing publish to topic: ${mapping.topicName}`, {
        duration: 2000
      })
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                        ...
                        <TableBody>
                          {mappings.map((mapping, index) => (
                            <TableRow key={mapping.id}>
                              ...
                              <TableCell>
                                <Badge variant="outline">{mapping.format}</Badge>
                              </TableCell>
                              <TableCell>
                              ...
                              <TableCell>{mapping.qos}</TableCell>
                              <TableCell>
                                <Badge variant="outline" className="text-xs">{mapping.format}</Badge>
                              </TableCell>
                              <TableCell>
                              ...
                              <TableCell className="text-right">
                                <div className="flex justify-end gap-2">
                                  ...
                                  <Button
                                    ...
                                    variant="outline"
                                    size="sm"
                                    onClick={() => testPublish(mapping.id)}
                                  >
              ...
              <TopicModal
                ...
                topic={editingMappingIndex !== null ? mappings[editingMappingIndex] : null}
                onSave={saveMapping}
                onOpenTagSelector={() => {
                  if (editingMappingIndex !== null) {
                    openTagSelector(editingMappingIndex)
                  }
                }}
              />
