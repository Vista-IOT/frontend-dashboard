...
@app.delete('/mappings/modbus/{data_id}')
def delete_modbus_mapping(data_id: str):
    """Delete a specific Modbus mapping by data_id"""
    try:
        ...
        # Remove from MODBUS_MAPPING
        MODBUS_MAPPING.remove_mapping(data_id)
...
# ==================== Calculation Tag Endpoints ====================

@app.post('/calculation-tags')
def register_calculation_tag(body: dict):
    """
    Register a calculation tag
    
    Body:
        name: str - Tag name
        formula: str - Formula expression (e.g., "A + B * 2")
        variables: dict - Variable mappings (e.g., {"A": "rohan10", "B": "rohan20"})
        default_value: float - Initial value (default: 0.0)
        period: int - Update period in seconds (default: 1)
    """
    ...
    if not isinstance(formula, str) or formula == '':
        raise HTTPException(400, 'formula required')
    if not isinstance(variables, dict):
        raise HTTPException(400, 'variables must be a dict')
    
    try:
        CALCULATION_ENGINE.register_calculation_tag(
            tag_name, formula, variables, default_value, period
        )
        return {'ok': True, 'name': tag_name}
    except Exception as e:
        raise HTTPException(400, str(e))


@app.delete('/calculation-tags/{tag_name}')
def unregister_calculation_tag(tag_name: str):
    """Unregister a calculation tag"""
    try:
        CALCULATION_ENGINE.unregister_calculation_tag(tag_name)
        return {'ok': True}
    except Exception as e:
        raise HTTPException(400, str(e))


@app.get('/calculation-tags')
def get_calculation_tags():
    """Get all registered calculation tags"""
    return CALCULATION_ENGINE.get_status()


@app.post('/calculation-tags/evaluate')
def evaluate_calculation_tags():
    """Manually trigger evaluation of all calculation tags"""
    try:
        CALCULATION_ENGINE.evaluate_all()
        return {'ok': True}
    except Exception as e:
