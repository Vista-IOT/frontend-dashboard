...
class DataStore:
    ...
    def unregister(self, key: str) -> bool:
        """
        Unregister a data point and clean up all associated mappings and history.
        
        Args:
            key: The key of the data point to unregister
            
        Returns:
            bool: True if unregistered successfully, False if key not found
        """
        with self._lock:
            # Check if key exists
            if key not in self._data_points:
            ...
            # Remove address mapping if exists
            if dp.address is not None and dp.address in self._address_to_key:
                del self._address_to_key[dp.address]
            
            # Remove key-to-address mapping
            if key in self._key_to_address:
                del self._key_to_address[key]
            
            # Remove ID mappings
            if key in self._key_to_id:
                data_id = self._key_to_id[key]
                del self._key_to_id[key]
                if data_id in self._id_to_key:
                    del self._id_to_key[data_id]
