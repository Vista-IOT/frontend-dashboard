        ...
        while not self._stop_event.is_set():
            try:
                ...
                for device_name, tags in polled_values.items():
                    # Skip if tags is not a dict (handles direct tag name mappings)
                    ...
                    for tag_id, tag_data in tags.items():
                        ...
                        # Push successful reads with valid values
                        # Include: SUCCESS, success, ok, OK, good (for virtual tags)
                        if status in ["SUCCESS", "success", "ok", "OK", "good"] and value is not None:
                            try:
                                # Determine the key format based on tag source
                                source = tag_data.get('source', '')
                                
                                if source == 'user_tag':
                                    # User tags: use tag name directly (no device prefix)
                                    full_key = tag_name
                                elif source == 'calculation_tag':
                                    # Calc tags: use tag name directly (no calc: prefix)
                                    full_key = tag_name.replace('calc:', '')
                                elif device_name in ['USER_TAGS', 'CALC_TAGS']:
                                    # Virtual tag devices: use tag_name directly
                                    full_key = tag_name
                                else:
                                    # IO tags: use device_name:tag_name format
